...
//at the end of each block, information about it
typedef struct
{
    int numOfRecords; //number of records the current block has
}HP_block_info;

/* Η δομή HP_info κρατάει μεταδεδομένα που σχετίζονται με το αρχείο σωρού*/
//The first block of evey heap file must hold the data of the file
//it is pinned the whole time until we close the file
//if it is dirty we keep it in memory till the file is closed
//so if something changes in the heap file
//our block is set to the state of pinned and dirty!
typedef struct
{
    int maxRecordsPerBlock;     //it saves the number of records that every block can have (8)
    int totalRecords;
    int totalBlocks;
    int lastBlockID;
